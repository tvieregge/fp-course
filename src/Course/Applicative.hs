...
instance Applicative Optional where
  ...
  (<*>) ::
    ...
    -> Optional a
    -> Optional b
  (<*>) f a = bindOptional (flip P.fmap a) f
...
sequence ::
  ...
  List (f a)
  -> f (List a)
sequence lfa = foldRight ( lift2 (:.)  ) (pure Nil) lfa
...
filtering ::
  ...
  -> List a
  -> f (List a)
filtering p xs = foldRight f (pure Nil) xs
    where f a fxs = lift2 (\b as -> if b then a :. else as) (p a) fxs
